...
export class PerspectiveDocumentWidget extends DocumentWidget {
  ...
  async _update() {
    ...
    this._psp.theme = document.body.getAttribute("data-jp-theme-light") === "false" ? "Pro Light" : "Pro Dark";
  }
...
async function activate(app, restorer, themeManager) {
  ...
  if (restorer) {
    ...
    restorer.restore(trackerparquet, {
  ...
  factoryparquet.widgetCreated.connect((sender, widget) => {
    // Track the widget.
    trackerparquet.add(widget);

    // Notify the widget tracker if restore data needs to update.
    widget.context.pathChanged.connect(() => {
      trackerparquet.save(widget);
    });
